...
1. **If Atlassian MCP tools are available** (e.g. Jira issue search, query): Follow **expo-atlassian-integration** skill — query Jira for open issues, list them, suggest the next one, and optionally mention PROJECT_BRIEF.md progress for context.
2. **If Atlassian MCP is not available**, use the brief only:
